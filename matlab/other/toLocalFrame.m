function outVector = toLocalFrame(p,phi,inVector)
...
% (in case only 1 state/position: in column format).
...
for i = 1:size(inVector,2)
    temp             = inv(Tr)*[inVector(1:2,i);1];
    outVector(1:2,i) = temp(1:2);
    if adaptAngles
        outVector(3,i) = inVector(3,i) - phi;
    end
end