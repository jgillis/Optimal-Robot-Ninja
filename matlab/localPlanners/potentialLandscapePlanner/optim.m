function localPlanner = optim(MPC,localPlanner,globalPlanner,veh,MPC_iteration)
...
%         x_init1  = [linspace(0,0,100);linspace(0,6,100); ...
%             zeros(1,100)];
%         x_init2  = [linspace(0,x_final(1),101);linspace(6,x_final(2),101); ...
%             linspace(0,x_final(3),101)];
%         x_init = [x_init1 x_init2];

%         x_init1 = interpolateUntil(globalPlanner.worldCoordinates(:,1),n+1);
%         x_init2 = interpolateUntil(globalPlanner.worldCoordinates(:,2),n+1);
...
elseif strcmp(solver,'sqp1')==1
    ...
    if MPC.k<3
        ...
        if localPlanner.goalInView
            fprintf('\t Goal in view \n');
            problem    = localPlanner.solver.problemSqp1B;
        else
            fprintf('\t Goal not in view \n');
            problem    = localPlanner.solver.problemSqp1A;
        end
    end
elseif strcmp(solver,'sqp2')==1
    fprintf('\t \t Using SQP solver \n');
    if MPC.k<3
        % initial guesses for first iterations OR when close to goal
        phi         = atan2(x_final(3),n);
        ...
        if localPlanner.goalInView
            fprintf('\t Goal in view \n');
            problem    = localPlanner.solver.problemSqp2B;
        else
            fprintf('\t Goal not in view \n');
            problem    = localPlanner.solver.problemSqp2A;
        end
    end
elseif strcmp(solver,'sqpAll')==1
    fprintf('\t \t Using SQP solver \n');
    if MPC.k<3
        % initial guesses for first iterations OR when close to goal
        phi         = atan2(x_final(3),n);
        ...
        if localPlanner.goalInView
            fprintf('\t Goal in view \n');
            problem    = localPlanner.solver.problemSqpAllB;
        else
            fprintf('\t Goal not in view \n');
            problem    = localPlanner.solver.problemSqpAllA;
        end
    end
end
